#ifndef __CLOUD_TO_IMG_H_
#define __CLOUD_TO_IMG_H_
...
public:
    ...
    void cloud_handler(const sensor_msgs::PointCloud2ConstPtr &cloud_msg)
    {
        ...
        std::cout << "Laser points: " << laser_cloud->size() << std::endl;
        ...
        for (int u = 0; u < IMAGE_HEIGHT; u++) 
        {
            for (int v = 0; v < IMAGE_WIDTH; v++) 
            {
                ...
                double theta = std::atan2(pt.y,pt.z) / M_PI * 180 + 180;
                ...
                double dis2 = pt.z * pt.z + pt.y * pt.y;
                double theta2 = std::asin(sqrt(dis2/dis)) / M_PI * 180;

                float noise = pt.noise;
                float intensity = scanID+(pt.intensity/10000);

                std::cout << "laser intensity: " << intensity << std::endl;
        ...
        // tf_base_to_lidar.sendTransform(tf::StampedTransform(base_to_lidar, cloud_msg->header.stamp, "base_link", "laser"));
    }
