...
void odom_handler(const nav_msgs::Odometry  &odom_msg)
{
...
{
    cloudToImg->cloud_handler(cloud_msg);
    ...
    KeyFrame* keyframe = new KeyFrame(cloud_time,
                                      global_frame_index,
                                      cloudToImg->image_intensity,
                                      cloudToImg->cloud_track);   
...
int main(int argc, char **argv)
{
    ...
    n.getParam("loop_config_file", config_file);
    ...
    ros::Subscriber sub_path  = n.subscribe(PATH_TOPIC,  1, odom_handler);
